...
//-----------------------------------------------------------------------------------
    ...
    public function modelClassName(){
        $this->modelClassName = str_replace('Controller', '', get_called_class());
    }
    ...
    public function controllerUrl($action = '', array $args = array(), $langPrefix = false){
        $url = '';
        if($langPrefix){
            $url .= $this->langPrefix();
        }
        
        $url .= '/'.strtolower($this->modelClassName);
        if($action){
            $url .= '/'.$action;
        }
    ...
    public function printControllerUrl($action = '', array $args = array(), $langPrefix = false){
        return '/cp'.$this->controllerUrl($action, $args, $langPrefix);
    }
//-------------------------------------------------------------------------------------
    static $rules = array(
        'index' => array(
   ...
   function link($path = '') {
    ...
    //-------------Check on engine configuration---------------
    public function isConfigurate(){
        if(!Contr::theme()){
            Controller::gi()->redirect('/cp/configurate');
        }
    }
    //---------------------------------------------------------        
    //-------------Redirect on theme controller----------------
    public function themeRedirect(){
        $theme = Contr::theme();
        if($theme){
            $this->redirect('/cp/'.$theme->control);
        }
    }
    //---------------------------------------------------------   
    //--------------If engine is new---------------------------
    public function isNew() {
        $theme = Contr::theme();
        if($theme){
            if((time() - $theme->time) < 300){
                $this->redirect('/cp/configurate/common');
                exit();
            } 
        }
