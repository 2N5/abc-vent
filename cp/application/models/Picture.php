...
class Picture extends ModelTable{
    ...
    public $safe = array('id', 'name', 'alt', 'title', 'id_theme', 'is_main', 'is_isometry');
...
//        debug($this->files, true);
        ...
        $this->name = $fileName;
        $this->id_theme = $this->model->id;
        
        $pics = self::modelsWhere('id_theme = ?', array($this->id_theme));
        if(!count($pics))
        {
...
//        debug($this, true);
    ...
    public static function takePicture(ModelTable $model) {
        ...
        //Only theme picture
        $modelTheme = ucfirst(App::gi()->theme->control);
...
//        debug($pictures, true);
    ...
    public static function allExistObjects($id_cert){        
        return self::modelsWhere('id_theme = ?', array($id_cert));
    }
