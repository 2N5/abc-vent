...
class CertificateController extends Controller {
    ...
    public function actionCreate() {
        if(isset($_POST['form'])){
            ...
            $certificate->__attributes = $_POST['form'];
            $certificate->url = translit($certificate->title);
            $certificate->meta_description = AdditionalHelper::all(AdditionalHelper::GENERATE);
            if($certificate->save())
            {
               $this->redirect($this->printControllerUrl('edit', array($certificate->id))); 
            }
        }
    ...
    {
        $certificate = Certificate::model($id_cert);
        if(!$certificate)
        {
            $this->error404();
        }
        ...
        if(isset($_POST['field']))
        {
            ...
            $field->id_theme = $certificate->id;
            $field->save();
            
            $this->redirect($this->printControllerUrl('form', array($certificate->id)));
        }
    ...
    protected function takeExistObjects(){
        $this->existObjects = Certificate::allExistObjects();
    }
    ...
    protected function takeSingleExistObject($id){
        $this->existSingleObject = Certificate::singleExistObject($id);
        if(!$this->existSingleObject)
        {
            $this->redirect('/error/404');
        }
