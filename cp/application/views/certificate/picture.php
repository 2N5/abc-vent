<div class="col-md-10">
	Операции с картинками для ( <?= $this->existSingleObject->title; ?> )
	<form action="" method="POST" enctype="multipart/form-data">
		<?php if ($picture) {
			?>
			<a href="/#" class="btn alert-danger" style="position: absolute;top: -30px;right: 15px;"
			...
			<img src="<?= echoIMG($picture); ?>" style="width:75px; margin-bottom: 10px;" >
		...
		<div style="width: 300px;overflow: hidden;">
			<img src="<?= echoIMG($this->existSingleObject->picture); ?>" style="width: 150px; display: block;margin-bottom: 10px;">
		...
		<?php if (count($this->existSingleObject->album)) {
			foreach ($this->existSingleObject->album as $photo) {
				?>
				<div style="width: 300px;overflow: hidden;">
					<img src="<?= echoIMG($photo); ?>" style="width: 150px;display: block;">
					...
					<?php if ($this->existSingleObject->picture) {
						...
						<a href="<?= $this->printControllerUrl('pictureedit', array($this->existSingleObject->id, $photo->id)); ?>" class="btn alert-warning" style="width: auto;margin: 0 5px 5px 0; float: left;">Сделать главной</a>
						<?php } 
					}else{ ?>
					<a href="<?= $this->printControllerUrl('pictureedit', array($this->existSingleObject->id, $photo->id)); ?>" class="btn alert-warning" style="width: auto;margin: 0 5px 5px 0;float: left;">Сделать главной</a>
					<?php } ?> 
