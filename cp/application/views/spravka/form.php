<div class="col-md-10">
	Редактирование полей формы справки: <?=$spravka->title; ?>
	...
	<table class="table">
		...
		<tbody>
			<form method="POST"> 
				<tr style="background-color: #3a4962;">
					<td>
						<select name="field[type]">
							...
							<?php if(!Field::modelWhere('id_theme = ? AND type = ?', array($spravka->id, Field::FILE))){ ?>
				...
				<form method="POST" action="<?=$this->printControllerUrl('form', array($spravka->id, $field->id)); ?>"> 
					<tr>
						<td>
							<select name="field[type]">
							...
							<td>
								...
								if (confirm('Вы уверены, что хотите произвести удаление связи?')) {
									location.href ='<?= $this->printControllerUrl('deletefield', array($spravka->id, $field->id)); ?>';
								}"
								>Удалить</a>
							</td>
						</tr>
			...
			<h3>Образец формы данной справки</h3>
			<form action="#">
				Имя:
