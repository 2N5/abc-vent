<div class="col-md-10">
	Редактирование полей формы чека: <?=$check->title; ?>
	...
	<table class="table">
		...
		<tbody>
			<form method="POST"> 
				<tr style="background-color: #3a4962;">
					<td>
						<select name="field[type]">
							...
							<?php if(!Field::modelWhere('id_theme = ? AND type = ?', array($check->id, Field::FILE))){ ?>
				...
				<form method="POST" action="<?=$this->printControllerUrl('form', array($check->id, $field->id)); ?>"> 
					<tr>
						<td>
							<select name="field[type]">
							...
							<td>
								...
								if (confirm('Вы уверены, что хотите произвести удаление связи?')) {
									location.href ='<?= $this->printControllerUrl('deletefield', array($check->id, $field->id)); ?>';
								}"
								>Удалить</a>
							</td>
						</tr>
