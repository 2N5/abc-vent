...
abstract class ModelTable extends Model {
    ...
    public function siblings() {
        $sibling = new Sibling($this);
        $this->previous = $sibling->previousUrl();
        $this->next = $sibling->nextUrl();
    }
     ...
     * @return \static
     */
    static function modelsQuery($sql, $values = []) {
    ...
    function preShorts() {
        //debug($this->__attributes);
        foreach ($this->__attributes as $key => $val) {
            if ($key == 'id' or $key == 'url') {
                continue;
            }
            $this->$key = shortCats($val);
        }
        //debug($this->__attributes, true);
    }
...
//        debug($results, true);
    ...
    static function modelWhere($where = '', $values = array()) {
        $modelname = get_called_class();
//        debug('SELECT * FROM `' . app::gi()->config->db['pref'] . $modelname::$table . '` WHERE ' . $where);
